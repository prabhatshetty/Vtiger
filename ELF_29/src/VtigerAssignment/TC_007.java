...
public class TC_007 extends BaseTest {
	public void invoice() {
		...
		//step 9: Select the sales order
		homePagee.getSalesLookupImage().click();
		Set<String> allsaleswindowproduct = driver.getWindowHandles();
		allsaleswindowproduct.remove(parent);
		for(String windowIddd:allsaleswindowproduct) {
			driver.switchTo().window(windowIddd);
			Salespage sales = new Salespage(driver);
			sales.getSalesorderMotoG().click();
